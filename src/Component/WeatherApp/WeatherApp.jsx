...
import { Dialog, Group, Button } from "@mantine/core";
...
function WeatherApp() {
  ...
  const columns = [
    ...
    },
    {
      ...
      Cell: ({ cell }) => {
        const tempValue = cell.getValue();
        return (
          <Box
            sx={(theme) => ({
              backgroundColor:
                tempValue < 0
                  ? theme?.colors?.red?.[9] || "#FF0000"
                  : tempValue >= 0 && tempValue < 25
                  ? theme?.colors?.yellow?.[9] || "#FFFF00"
                  : theme?.colors?.green?.[9] || "#00FF00",
              borderRadius: "4px",
              color: "#fff",
              maxWidth: "9ch",
              padding: "4px",
  ...
  const table = useMantineReactTable({
    ...
    onFiltersChange: (filters) => {
      const tempFilter = filters.find((filter) => filter.id === "temp");
      if (tempFilter) {
        const { value } = tempFilter;
      }
    },
  });
    ...
    <div>
      <div className="container">
        <div className="top-bar">
          ...
          >
            <Text>Check the Country or City Name is correct</Text>
        ...
        <div style={{ padding: "20px" }}>
          ...
          {value === 2 && (
            ...
            <div className="temperature-graph-container">
              <h2 className="temperature-graph-heading">Temperature Graph</h2>
              {weatherData && (
